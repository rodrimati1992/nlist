...
impl<T, L: PeanoInt> NList<T, L> {
        ...
        T: Copy
    {
        match Self::WIT {
            ...
            NodeWit::Cons{len_te, ..} => {
                NList::cons_sub(elem, NList::repeat_copy(elem), len_te)
            }
        }
    }
    ...
    pub fn reverse(self) -> NList<T, L>  {
            ...
            const NEW: Self = match (LI::PEANO_WIT, peano::cmp_peanos(LA::NEW, LR::NEW)) {
                (PeanoWit::PlusOne(input_te), TypeCmp::Ne(_)) => {
                    let TypeCmp::Eq(output_te) = 
                        peano::cmp_peanos(
                            PlusOne::<LA>::NEW, 
                            peano::Min::<PlusOne<LA>, LR>::NEW
                        )
                    else {
                        const_panic::concat_panic!(
                            "somehow, LA > LR: ", LA::USIZE, " > ", LR::USIZE
                        )
                    };
                ...
                (PeanoWit::Zero(_), TypeCmp::Eq(output_te)) => {
                    ReverseState::Finished{
                        output_te: output_te.map(NListFn::NEW),
                    }
                }
                _ => const_panic::concat_panic!(
                    "somehow, ", LI::USIZE, " + ", LA::USIZE, "!=", LR::USIZE
                )
            };
        }
            ...
            LR: PeanoInt,
        {
            match ReverseState::<T, LI, LA, LR>::NEW {
                ReverseState::Iterating{input_te, output_te} => {
...
impl<T, L: PeanoInt> NList<T, L> {
    ...
    pub const fn coerce_length<L2: PeanoInt>(
        self,
        len_te: TypeEq<L, L2>
    ) -> NList<T, L2> {
...
const fn index_list<L: PeanoInt>() -> NList<usize, L> {
    match <NList<usize, L>>::WIT {
        ...
        NodeWit::Cons{len_te, ..} => {
            NList::cons_sub(L::USIZE, index_list(), len_te)
        }
    }
}