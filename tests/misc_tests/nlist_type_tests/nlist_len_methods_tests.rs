use nlist::{Peano, PeanoInt, PeanoWit, NList, nlist};
...
#[test]
fn len_test() {
    const fn callit<T, L: PeanoInt>(list: &NList<T, L>) -> usize {
        list.len()
    }
...
#[test]
fn is_empty_test() {
    const fn callit<T, L: PeanoInt>(list: &NList<T, L>) -> bool {
        list.is_empty()
    }
...
#[test]
fn len_proof_test() {
    const fn callit<T, L: PeanoInt>(list: &NList<T, L>) -> PeanoWit<L> {
        list.len_proof()
    }

    {
        let proof = callit(&nlist![0u8; 0]);
        assert_type::<PeanoWit<Peano!(0)>>(proof);
        assertm!(proof, PeanoWit::Zero{..});
    }
    {
        let proof = callit(&nlist![1u16]);
        assert_type::<PeanoWit<Peano!(1)>>(proof);
        assertm!(proof, PeanoWit::PlusOne{..});
    }
    {
        let proof = callit(&nlist![2u32, 3]);
        assert_type::<PeanoWit<Peano!(2)>>(proof);
        assertm!(proof, PeanoWit::PlusOne{..});
    }
    {
        let proof = callit(&nlist![5u64, 8, 13]);
        assert_type::<PeanoWit<Peano!(3)>>(proof);
        assertm!(proof, PeanoWit::PlusOne{..});
    }
}
